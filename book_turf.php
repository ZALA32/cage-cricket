...
// Fetch turf details (owner_id references users.id)
$query = "SELECT turf_id, turf_name, booking_cost, turf_capacity, turf_area, turf_photo, turf_address, is_active, owner_id 
          FROM turfs WHERE turf_id = ? AND is_active = 1";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $turf_id);
$stmt->execute();
$turf_result = $stmt->get_result();
if ($turf_result->num_rows === 0) {
    $_SESSION['dashboard_message'] = "<div class='alert alert-danger'>Turf not found or inactive.</div>";
    header("Location: turf_selection.php");
    exit;
}
...
// Generate all possible start times (30-minute intervals from 6:00 AM to 10:00 PM)
$start_times = [];
...
function isWeekend($date) {
    ...
    $day = $dateObj->format('N'); // 1 (Monday) to 7 (Sunday)
    return $day >= 6; // Saturday (6) or Sunday (7)
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['check_slots'])) {
    ...
    if (empty($errors)) {
        ...
        $stmt = $conn->prepare($query);
        $stmt->bind_param("isssssss", $turf_id, $date, $end_time, $start_time, $end_time, $start_time, $start_time, $end_time);
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    if (empty($errors)) {
        $query = "INSERT INTO bookings (turf_id, organizer_id, booking_name, date, start_time, end_time, total_audience, services, total_cost, status, created_at) 
                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', NOW())";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("iissssisd", $turf_id, $user_id, $booking_name, $date, $start_time, $end_time, $total_audience, $services_text, $total_cost);
        if ($stmt->execute()) {
            $booking_id = $conn->insert_id;

            // Insert payment record
            $query = "INSERT INTO payments (booking_id, payment_status, created_at) VALUES (?, 'pending', NOW())";
            ...
            $notification_message = "Your booking for {$turf['turf_name']} on $date has been submitted and is awaiting turf owner approval.";
            ...
            // Optionally, notify turf owner (users.id)
            $owner_id = $turf['owner_id'];
            $owner_notify_message = "A new booking for {$turf['turf_name']} on $date has been submitted and is awaiting your approval.";
            $query = "INSERT INTO notifications (user_id, message, is_read, created_at) VALUES (?, ?, 0, NOW())";
            ...
            $errors[] = "Error creating booking: " . $conn->error;
        }
        $stmt->close();
    }
}

$page_title = "Book Turf - Cage Cricket";
?>
...
<head>
    ...
    <link rel="stylesheet" href="css/book_turf.css">
</head>
<body>
        ...
        <div class="row justify-content-center">
            <div class="col-12 col-md-10 col-lg-10 col-xl-12 col-xxl-12 fade-in">
                ...
                <div class="header-section">
                    <div>
                        <h1>Book <?php echo htmlspecialchars($turf['turf_name']); ?></h1>
                        <p>Reserve Your Spot for an Unforgettable Match!</p>
                    </div>
                </div>
                ...
                <div class="booking-card">
                    <div class="booking-form-container">
                        <h4><i class="bi bi-ticket-perforated"></i> Book Your Slot</h4>
                            ...
                            <div class="col-12 fade-in-section">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" name="booking_name" value="<?php echo htmlspecialchars($_POST['booking_name'] ?? ''); ?>" class="form-control" placeholder="e.g., Team Practice" title="Enter your event name" required>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label"><i class="bi bi-person-arms-up"></i> Total Audience <span class="required-asterisk">*</span></label>
                                        <input type="number" name="total_audience" id="total_audience" value="<?php echo htmlspecialchars($_POST['total_audience'] ?? ''); ?>" class="form-control" placeholder="e.g., <?php echo min(5, $turf['turf_capacity']); ?>" min="1" max="<?php echo $turf['turf_capacity']; ?>" title="Number of attendees (max <?php echo $turf['turf_capacity']; ?>)" required>
                            ...
                            <div class="col-12 fade-in-section">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" name="date" id="date-picker" value="<?php echo htmlspecialchars($_POST['date'] ?? ''); ?>" class="form-control flatpickr-input" placeholder="Select date" title="Choose your booking date" required>
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <select name="start_time" id="start-time" class="form-select" title="Select the start time for your booking" required>
                                            ...
                                            <?php foreach ($start_times as $time): ?>
                                                <option value="<?php echo $time; ?>" <?php echo (isset($_POST['start_time']) && $_POST['start_time'] === $time) ? 'selected' : ''; ?>>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select name="duration" id="duration" class="form-select" title="Select how long you need the turf" required>
                                            ...
                                            <?php foreach ($durations as $hour): ?>
                                                <option value="<?php echo $hour; ?>" <?php echo (isset($_POST['duration']) && (int)$_POST['duration'] === $hour) ? 'selected' : ''; ?>>
                                ...
                                <div class="services-checkboxes">
                                    <?php foreach ($services_options as $key => $service): ?>
                                        <div class="form-check form-check-inline">
                                            <input type="checkbox" name="services[]" value="<?php echo $key; ?>" class="form-check-input" id="service_<?php echo $key; ?>" <?php echo in_array($key, $_POST['services'] ?? []) ? 'checked' : ''; ?>>
                            ...
                            <div class="col-12 fade-in-section">
                                <div class="button-container">
                                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#confirmModal">
                                        <i class="bi bi-check-circle-fill"></i> Book Turf
                                    </button>
                                    <a href="turf_selection.php" class="btn btn-primary">
                                        <i class="bi bi-search"></i> Browse Other Turfs
                                    </a>
                                </div>
                            </div>
                        </form>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="confirmModalLabel"><i class="bi bi-check-circle-fill"></i> Confirm Booking</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>
                            <div class="modal-body">
                                Are you sure you want to submit this booking for <?php echo htmlspecialchars($turf['turf_name']); ?>?
